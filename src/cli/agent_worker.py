...
async def run_worker(room_name: str, agent_id: str, livekit_url: str, api_key: str, api_secret: str):
    logger.info("🚀 Starting Standalone Agent Worker")
    ...
    try:
        ...
        session_data = await create_agent_session(agent_id=agent_id, room_name=room_name, token=token, livekit_url=livekit_url)
