"""
Stimm wrapper specific configuration.

This config only contains settings specific to the stimm wrapper.
...
class StimmConfig:
    ...
    VAD_THRESHOLD: float = float(os.getenv("STIMM_VAD_THRESHOLD", "0.01"))
    SILENCE_TIMEOUT_MS: int = int(os.getenv("STIMM_SILENCE_TIMEOUT_MS", "1000"))
    MIN_SPEECH_DURATION_MS: int = int(os.getenv("STIMM_MIN_SPEECH_DURATION_MS", "300"))

    # WebRTC VAD settings (for backend VAD processing)
    WEBRTC_VAD_AGGRESSIVENESS: int = int(os.getenv("STIMM_WEBRTC_VAD_AGGRESSIVENESS", "2"))
