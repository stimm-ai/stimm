...
from database import Agent, AgentSession

from .agent_service import AgentService
from .exceptions import AgentNotFoundError
from .models import AgentConfig, AgentSessionCreate
...
class AgentManager:
    ...
    def _clean_cache(self):
        ...
        expired_keys = [agent_id for agent_id, timestamp in self._cache_timestamps.items() if current_time - timestamp > self._cache_ttl]
    ...
    def get_agent_config_by_name(self, agent_name: str) -> AgentConfig:
        ...
        try:
            ...
            agent = session.query(Agent).filter(Agent.user_id == system_user_id, Agent.name == agent_name, Agent.is_active).first()
    ...
    def get_session_agent(self, session_id: str) -> AgentConfig:
        ...
        try:
            ...
            agent_session = session.query(AgentSession).filter(AgentSession.id == UUID(session_id), AgentSession.expires_at > datetime.utcnow()).first()
    ...
    def end_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            agent_session = session.query(AgentSession).filter(AgentSession.id == UUID(session_id)).first()
    ...
    def get_active_sessions_count(self) -> int:
        ...
        should_close = self.db_session is None
        try:
            count = session.query(AgentSession).filter(AgentSession.expires_at > datetime.utcnow()).count()
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        try:
            ...
            result = session.query(AgentSession).filter(AgentSession.expires_at <= datetime.utcnow()).delete()

            session.commit()

            # Clean active sessions cache
            current_time = time.time()
            expired_sessions = [session_id for session_id, timestamp in self._cache_timestamps.items() if current_time - timestamp > self._cache_ttl]
