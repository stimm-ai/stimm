...
from .models import AgentCreate, AgentResponse, AgentUpdate
...
async def get_default_agent(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No default agent found")


@router.put("/{agent_id}/set-default", response_model=AgentResponse)
async def set_default_agent(agent_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to set default agent")


@router.get("/{agent_id}/providers", response_model=dict)
async def get_agent_providers(agent_id: str, db: Session = Depends(get_db)):
...
async def update_agent_providers(agent_id: str, provider_configs: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update agent providers")
...
async def get_available_providers():
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to load available providers")
