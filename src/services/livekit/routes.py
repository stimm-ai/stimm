...
async def create_livekit_room(request: CreateRoomRequest):
    ...
    try:
        ...
        room_info = await livekit_service.create_room_for_agent(request.agent_id, request.room_name)
...
async def job_notification(request: JobNotificationRequest):
    ...
    try:
        ...
        return {"status": "success", "message": f"Agent {request.agent_id} notified to join room {request.room_name}"}
...
async def cleanup_sip_rooms():
    ...
    try:
        ...
        return {"status": "success", "message": f"Deleted {deleted} SIP rooms and terminated agent processes"}
