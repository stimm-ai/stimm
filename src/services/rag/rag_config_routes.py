...
async def list_rag_configs(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def get_rag_config(rag_config_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"RAG configuration with ID {rag_config_id} not found")


@router.post("/", response_model=RagConfigResponse, status_code=status.HTTP_201_CREATED)
async def create_rag_config(rag_config_data: RagConfigCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"message": str(e), "errors": [str(e)]})
    except AgentAlreadyExistsError as e:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail={"message": str(e), "errors": [str(e)]})
    except Exception as e:
...
async def update_rag_config(rag_config_id: str, rag_config_data: RagConfigUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update RAG configuration")


@router.delete("/{rag_config_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_rag_config(rag_config_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete RAG configuration")


@router.get("/default/current", response_model=RagConfigResponse)
async def get_default_rag_config(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No default RAG configuration found")


@router.put("/{rag_config_id}/set-default", response_model=RagConfigResponse)
async def set_default_rag_config(rag_config_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to set default RAG configuration")
...
async def get_available_rag_providers():
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to load available RAG providers")
...
async def upload_documents(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"RAG configuration {rag_config_id} not found")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to initialize RetrievalEngine: {str(e)}")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to ensure collection: {str(e)}")
...
async def list_documents(rag_config_id: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def delete_document(rag_config_id: str, document_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if str(document.rag_config_id) != rag_config_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Document does not belong to this RAG configuration")
...
async def delete_all_documents(rag_config_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete all documents: {str(e)}")


@router.get("/{rag_config_id}/documents/statistics")
async def get_documents_statistics(rag_config_id: str, db: Session = Depends(get_db)):
