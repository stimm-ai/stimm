...
def preload_models():
    ...
    try:
        ...
        if retrieval_config.enable_reranker:
            ...
            reranker = CrossEncoder(retrieval_config.reranker_model)
...
def verify_model_cache():
    ...
    try:
        ...
        try:
            embed_cache_path = snapshot_download(
                repo_id=retrieval_config.embed_model_name,
        ...
        if retrieval_config.enable_reranker:
            try:
                rerank_cache_path = snapshot_download(
                    repo_id=retrieval_config.reranker_model,
                    local_files_only=True
                )
