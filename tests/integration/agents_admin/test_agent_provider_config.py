...
def test_llm_provider_validation_groq(agent_service, valid_provider_configs):
    ...
    groq_config = ProviderConfig(provider="groq.com", config={"api_key": "test_key", "model": "llama-3.3-70b-versatile", "temperature": 0.7, "max_tokens": 1000})

    agent_data = AgentCreate(name=test_name, description="Test Groq LLM", llm_config=groq_config, tts_config=valid_provider_configs["tts"], stt_config=valid_provider_configs["stt"], is_default=False)
...
def test_stt_provider_validation_deepgram(agent_service, valid_provider_configs):
    ...
    deepgram_config = ProviderConfig(provider="deepgram.com", config={"api_key": "test_key", "model": "nova-2", "language": "en-US"})

    agent_data = AgentCreate(
        name=test_name, description="Test Deepgram STT", llm_config=valid_provider_configs["llm"], tts_config=valid_provider_configs["tts"], stt_config=deepgram_config, is_default=False
    )
...
def test_stt_provider_validation_whisper(agent_service, valid_provider_configs):
    ...
    whisper_config = ProviderConfig(provider="whisper.local", config={"base_url": "http://localhost:8000", "model": "base"})

    agent_data = AgentCreate(
        name=test_name, description="Test Whisper STT", llm_config=valid_provider_configs["llm"], tts_config=valid_provider_configs["tts"], stt_config=whisper_config, is_default=False
    )
...
def test_tts_provider_validation_elevenlabs(agent_service, valid_provider_configs):
    ...
    elevenlabs_config = ProviderConfig(provider="elevenlabs.io", config={"api_key": "test_key", "voice": "test_voice", "model": "eleven_turbo_v2_5"})

    agent_data = AgentCreate(
        name=test_name, description="Test ElevenLabs TTS", llm_config=valid_provider_configs["llm"], tts_config=elevenlabs_config, stt_config=valid_provider_configs["stt"], is_default=False
    )
...
def test_multiple_provider_combinations(agent_service):
    ...
    agent_data = AgentCreate(
        ...
        llm_config=ProviderConfig(provider="groq.com", config={"api_key": "test", "model": "llama-3.3-70b-versatile"}),
        tts_config=ProviderConfig(provider="elevenlabs.io", config={"api_key": "test", "voice": "test", "model": "eleven_turbo_v2_5"}),
        stt_config=ProviderConfig(provider="deepgram.com", config={"api_key": "test", "model": "nova-2"}),
        is_default=False,
    )
